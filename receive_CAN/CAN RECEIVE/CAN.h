...
void receive_all_messages(){
	changeBits(RXB0CTRL, 1<<RXM0, 1<<RXM0);
	changeBits(RXB0CTRL, 1<<RXM1, 1<<RXM1);
}

void enable_rollover(){
	changeBits(RXB0CTRL, 1<<RXB0CTRL, 1<<RXB0CTRL);
}
...
//filter zero is used for rxb0 receive buffer(and rxb1 if bukt/rollover is set)
void set_filter0(uint16_t identifier){
	writeRegister(RXF0SIDH, (uint8_t)(identifier>>3));
	writeRegister(RXF0SIDL, (uint8_t)(identifier<<5));
}
//filter one is used for rxb0 receive buffer(and rxb1 if bukt/rollover is set)
void set_filter1(uint16_t identifier){
	writeRegister(RXF1SIDH, (uint8_t)(identifier>>3));
	writeRegister(RXF1SIDL, (uint8_t)(identifier<<5));
}
//filter two is used for rxb1 receive buffer
void set_filter2(uint16_t identifier){
	writeRegister(RXF2SIDH, (uint8_t)(identifier>>3));
	writeRegister(RXF2SIDL, (uint8_t)(identifier<<5));
}
//filter three is used for rxb1 receive buffer
void set_filter3(uint16_t identifier){
	writeRegister(RXF3SIDH, (uint8_t)(identifier>>3));
	writeRegister(RXF3SIDL, (uint8_t)(identifier<<5));
}
//filter four is used for rxb1 receive buffer
void set_filter4(uint16_t identifier){
	writeRegister(RXF4SIDH, (uint8_t)(identifier>>3));
	writeRegister(RXF4SIDL, (uint8_t)(identifier<<5));
}
//filter zero is used for rxb1 receive buffer
void set_filter5(uint16_t identifier){
...
{
	/* Initialize external interrupt on pin INT0 on Rising edge */
	//ATMEGA SIDE
	EICRA |= (1 << ISC01);
	EICRA |= (1<<ISC00);
	EIMSK |= (1 << INT0);
	...
	changeBits(CANINTE, 1<<RX1IE, 0x01);  //enable receive interrupt on RXB1
...
void init_MCP2515(void)
{
	...
	spiMasterINIT();
	/* Initialize external interrupt service on mcp2515 and atmega38p */
