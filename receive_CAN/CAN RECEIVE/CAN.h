...
bool setFilter(uint8_t filter_num, uint16_t identifier)
{
	switch (filter_num)
	{
		case 0:
...
void filters_off()
{
	changeBits(RXB0CTRL, (1<<RXM0),1);
	changeBits(RXB0CTRL, (1<<RXM1),1);
	changeBits(RXB1CTRL, (1<<RXM0),1);
	changeBits(RXB1CTRL, (1<<RXM1),1);
}
